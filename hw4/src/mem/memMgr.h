...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* n_list = &_recycleList[m];
      while (n_list != NULL)
      {
         if (n_list -> getArrSize() == n)
         {
            return n_list;
         }
         if (n_list -> _nextList == NULL)
         {
            n_list -> _nextList = new MemRecycleList<T>(n);
         }
         n_list = n_list -> _nextList;
      }
      //n_list = new MemRecycleList<T>(n);
      return n_list;
   }
   ...
   T* getMem(size_t t) {
      ...
      if (t > _blockSize)
      {
         cerr << "Requested memory (" << t << ") is greater than block size(" << _blockSize << "). Exception raised..." << endl;
         throw bad_alloc();
      }
      ...
      // TODO
      MemRecycleList<T>* n_list = getMemRecycleList(n);
      ret = n_list -> _first;
      #ifdef MEM_DEBUG
      cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
      #endif
